...
# 2. 读取 V0.0（mb1, 偏移0, 1个字节byte）
    data = plc.mb_read(offset, bytes_read)  # 读1字节就够，因为开关量只占1位
...
# 2. 读取 V0.0（mb1, 偏移0, 1个字节byte）
    data = plc.mb_read(offset, bytes_write)  # 读1字节就够，因为开关量只占1位
...
# 4. 把修改后的整个字节写回去
    plc.mb_write(bytes_write, offset, data)
...
# 2. 读取 V0.0（mb1, 偏移0, 4个字节byte）
    data = plc.mb_read(offset, bytes_read)  # 读4字节，浮点量4个字节
...
# 2. 读取 V0.0（DB1, 偏移0, 1个字节byte）
    data = plc.mb_read(offset, bytes_read)  # 读2字节，word变量2字节
